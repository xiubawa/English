...
// Explicit POS table for the photo vocabulary. This avoids suffix guesses such as treating "acknowledgement" as an adjective.
const photoManualWordPosMap = {
  "abandon": "adjective",
  "abandoned": "adjective",
  "ability": "noun",
  "able": "adjective",
...
function photoManualWordRole(word) {
  return photoManualWordPosMap[cleanPhotoWord(word)] || "";
}
...
function photoWordRole(word, meaning) {
  const key = cleanPhotoWord(word);
  const manualRole = photoManualWordRole(key);
  ...
  if (photoAdjectiveWords.has(key)) return "adjective";
  if (isNounLikeWord(key, meaning)) return "noun";
  if (photoVerbWords.has(key)) return "verb";
  if (isReliableAdjectiveLikeWord(key)) return "adjective";
  return "noun";
}
...
function isNounLikeWord(word, meaning = "") {
  const key = cleanPhotoWord(word);
  return /(ment|tion|sion|ness|ity|cy|ance|ence|ship|hood|ism|th|age|ery|ry|ment|tude|sis|ics|ure)$/.test(key);
}

function isReliableAdjectiveLikeWord(word) {
  const key = cleanPhotoWord(word);
  return /(able|ible|ive|less|ous|ful)$/.test(key);
}
...
const photoFunctionWordPosMap = {
  ...
  about: "preposition", above: "preposition", across: "preposition", after: "preposition", against: "preposition", along: "preposition",
  among: "preposition", around: "preposition", at: "preposition", before: "preposition", behind: "preposition", between: "preposition",
  by: "preposition", despite: "preposition", during: "preposition", except: "preposition", for: "preposition", from: "preposition", in: "preposition",
  into: "preposition", near: "preposition", of: "preposition", on: "preposition", over: "preposition",
  through: "preposition", throughout: "preposition", to: "preposition", toward: "preposition", under: "preposition", upon: "preposition", via: "preposition",
...
const exampleWordTranslations = {
  ...
  important: "重要的", information: "信息", inspection: "检查", installing: "安装", international: "国际的", into: "进入；放入", invoice: "发票",
...
function exampleWordPartOfSpeech(word, matchedItem, meaning, candidates = []) {
  ...
  const manualRole = photoManualWordRole(word);
  if (manualRole) return photoRoleLabels[manualRole] || "??";
